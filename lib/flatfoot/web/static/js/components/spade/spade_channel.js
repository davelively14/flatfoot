...
import SpadeMain from './spade_main';
...
class SpadeChannel extends React.Component {
  ...
  renderActiveTab(text) {
    console.log(text);
    return (
      <li role="presentation" className="active"><a>{text}</a></li>
    );
  }

  renderInactiveTab(text) {
    return (
      <li role="presentation"><a className="text-link" onClick={() => this.props.setDashboardTab(text)}>{text}</a></li>
    );
  }
